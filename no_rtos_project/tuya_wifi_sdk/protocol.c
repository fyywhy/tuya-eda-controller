...
void all_data_update(void)
{
	...
	float temp = rand()%(295-225+1)+225;
	float humi = rand()%(800-600+1)+600;
	float batt = rand()%(850-650+1)+650;

    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    mcu_dp_value_update(DPID_TEMP_CURRENT,temp); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,humi / 10.0f); //VALUE型数据上报;
    //mcu_dp_enum_update(DPID_BATTERY_STATE,当前电池电量状态); //枚举型数据上报;
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,batt / 10.0f); //VALUE型数据上报;
...
//    mcu_dp_value_update(DPID_TEMP_SET,当前温度设置); //VALUE型数据上报;
	...
	printf("cur temp:%2f℃ humi:%2fRH batt:%2f\r\n",(temp/10.0f),(humi/10.0f),(batt / 10.0f));
}
...
void wifi_test_result(unsigned char result,unsigned char rssi)
{
    ...
    if(result == 0) {
        //测试失败
		printf("wifi test failed \r\n");
        if(rssi == 0x00) {
            //未扫描到名称为tuya_mdev_test路由器,请检查
			printf("wifi can't scan wifi named tuya_mdev \r\n");
        }else if(rssi == 0x01) {
            //模块未授权
			printf("wifi test failed,mo kuai wei shou quan \r\n");
        }
    }else {
        //测试成功
...
void mcu_get_mac(unsigned char mac[])
{
   ...
   */
    ...
    if(mac[0] == 1) {
        printf("get wifi mac failed.\r\n");
    }else {
        //正确接收到wifi模块返回的mac地址
		printf("\r\n x-get mac:%x-%x-%x-%x-%x-%x\r\n",mac[1],mac[2],mac[3],mac[4],mac[5],mac[6]);
		printf("\r\n c-get mac:%c-%c-%c-%c-%c-%c\r\n",mac[1],mac[2],mac[3],mac[4],mac[5],mac[6]);
		printf("\r\n d-get mac:%d-%d-%d-%d-%d-%d\r\n",mac[1],mac[2],mac[3],mac[4],mac[5],mac[6]);
		printf("mac:%s",mac);
    }
}
#endif
